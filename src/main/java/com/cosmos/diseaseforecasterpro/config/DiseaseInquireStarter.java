...
@Component
public class DiseaseInquireStarter {

    private Process process;

    @PostConstruct
    public void startUvicorn() {
        try {
            // 构造完整的命令，先切换目录再执行 uvicorn
            String command = "cmd /c cd /d D:\\PythonProject\\kbmedical\\KBMEDICAL && uvicorn inquire_service:app --reload";
                ...
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()))) {
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        if (line.contains("Uvicorn running on")) { // 检测启动日志
                            isStarted = true;
                            System.out.println("Uvicorn 疾病查询服务已成功启动");
                            break;
                        }
                    }
                    if (!isStarted) {
                        System.err.println("Uvicorn 疾病查询服务启动失败");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
            ...
            System.out.println("正在启动 Uvicorn 疾病查询服务...,请稍等");
        } catch (Exception e) {
            System.err.println("启动 Uvicorn 疾病查询服务失败: " + e.getMessage());
            e.printStackTrace();
        }
    }

    @PreDestroy
    public void stopUvicorn() {
        try {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.contains("python.exe")) {
                    ...
                    killProcess.waitFor();
                    System.out.println("Uvicorn 疾病查询服务已关闭，PID: " + pid);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
